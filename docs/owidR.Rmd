...
internet %>% filter(entity == "World") %>%
  ...
  theme_owid() # color, background, etc., are set in owid format.
```
...
internet %>% 
  ...
  theme_owid() # The labels argument allows you to make it clear that the value is a percentage
```
