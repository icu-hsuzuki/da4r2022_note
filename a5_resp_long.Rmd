...
```{r}
df_q1 %>% drop_na(gini) %>% filter(income == "Aggregates") %>% distinct(country)
```


```{r}
df_q1 %>% filter(country == "South Asia") %>% drop_na(gini) %>%
  ggplot(aes(year, gini)) + geom_line()
```


```{r cach = TRUE}
library(WDI)
WDIdata <- WDI(country =c("FR","SE","JP","US"),indicator=c("SG.GEN.PARL.ZS","NY.GDP.MKTP.KD.ZG","SI.POV.GINI"),start=2000,end=2021,extra=FALSE,cache=NULL)
WDIdata
```
...
## Boxplot, missing value, country

```{r}
ggplot(WDIdata,aes(x=year,y=SI.POV.GINI,color=country))+geom_line()+labs(title="Gini index by country, 2000 to 2021",x="year", y="Gini index")
```

```{r}
Model0 <-  WDIdata %>% lm(SI.POV.GINI~SG.GEN.PARL.ZS,.)
Model0 %>% broom::augment()
```

```{r cash = TRUE}
wdi_world1 <- WDI(country = "all", indicator = c(atm = "FB.ATM.TOTL.P5", gdppercap = "NY.GDP.PCAP.CD", pop = "SP.POP.TOTL", research = "GB.XPD.RSDV.GD.ZS", military = "MS.MIL.XPND.GD.ZS", education = "SE.XPD.TOTL.GD.ZS"), 1990, extra = TRUE, cache = wdi_cache)
```

```{r}
mod_t <- lm(atm ~ research, wdi_world1); mod_t
```
```{r}
summary(mod_t)
```
...
```{r cash = TRUE}
totalunemp <- WDI(country = "all", indicator = "SL.UEM.TOTL.ZS", start = 1991, end = NULL, extra = TRUE, cache = wdi_cache) 
```
...
```{r}
totalunemp %>% drop_na(SL.UEM.TOTL.ZS) %>% 
  group_by(year) %>% summarize(total = sum(SL.UEM.TOTL.ZS))
```

```{r cash = TRUE}
refugee_data <- WDI(country = "all", indicator = "SM.POP.REFG.OR", start = 1960, end = NULL, extra = FALSE,
    cache = NULL)
refugee_data
```

```{r}
refugee_data %>% glimpse()
```

```{r}
ggplot(data = refugee_data %>% 
  filter(country %in% c("Germany", "Italy", "Spain", "France"), year %in% 2010:2022)) +
  geom_histogram(mapping = aes(x = SM.POP.REFG.OR, fill = country), binwidth = 10, boundary = 0) + 
  labs(title = "Annual Estimate of Refugees in the Country (2010-2022)",
         subtitle = "Countries are hosting a differnt number of refugees. Germany shows high intake of refugees before the turn of the century.", 
       caption = "Data retrieved from the World Bank's WDI Dataset", x = "Number of Refugees in the Country", y = "count")
```

#### Forest area

```{r cash = TRUE}
forest <- WDI(country = "all", indicator = c("AG.LND.FRST.K2"), start = 1990, end = 2020, extra = TRUE, cache = NULL) #cache = wdi_cache
```

```{r}
forest %>% filter(region %in% c("East Asia & Pacific", "Europe & Central Asia", "Latin America & Caribbean", "Middle East & North Africa", "North America", "South Asia", "Sub-Saharan Africa") & (year == 1990)) %>%
  ggplot(aes(x = region, y = AG.LND.FRST.K2, color = region, fill = region)) + 
  geom_col() + labs(title = "Graph 1. Forest areas in 1990", subtitle = "Regions of the world", x = "Year", y = "Forest area (sq. km)") + scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 7))
```

```{r}
forest %>% filter(region %in% c("East Asia & Pacific", "Europe & Central Asia", "Latin America & Caribbean", "Middle East & North Africa", "North America", "South Asia", "Sub-Saharan Africa") & (year == 2020)) %>%
  ggplot(aes(x = region, y = AG.LND.FRST.K2, color = region, fill = region)) + 
  geom_col() + labs(title = "Graph 2. Forest areas in 2020", subtitle = "Regions of the world", x = "Year", y = "Forest area (sq. km)") + scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 7))
```


```{r}
region_list <- wdi_cache$country %>% filter(region != "Aggregates") %>% distinct(region) %>% pull()
REGION <- wdi_cache$country %>% filter(country %in% region_list)  %>% pull(iso2c)
# data.frame(region = region_list, iso2c = REGION)
region_list
REGION
wdi_cache$country %>% filter(iso2c %in% REGION) %>% distinct(country)
```


need to study models more
