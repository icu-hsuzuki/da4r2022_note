...
```{r}
by_ind <- data_rev %>% group_by(ind) %>% nest()
by_ind
```
...
```{r}
models <- map(by_ind$data, ind_model)
```
...
```{r}
models[[1]]$coefficients %>% as.list() %>% as_tibble()%>% rename(c("intercept" = "(Intercept)", "slope" = "value" ))
```
...
```
```{r}
rename(as_tibble(as.list(lm(cars)$coefficients)), "intercept"="(Intercept)")
```
```{r}
...
```{r}
as_tibble(as.list(lm(cars)$coefficients))
```

```{r}
broom::tidy(lm(cars)) %>% select(1:2) %>% pivot_wider(names_from = term, values_from = estimate)
```

```{r}
model_quality <- by_ind %>% 
  mutate(glance = map(model, broom::glance)) %>% 
  mutate()
  unnest(glance, .drop = TRUE)
glance
